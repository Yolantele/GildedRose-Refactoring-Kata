...
class Shop {
  ...
  updatePostExpiry(item) {
    if (item.name === brie) {item.quality += 1;}
  ...
  updateQuality() {
    ...
    this.items.forEach(item => {
      if (item.quality < 50 && item.quality > 0) {
        if (that.isRegular(item.name)) { that.updateRegular(item);}
        if (item.name === backPass) { that.updateBackPass(item);}
        if (item.sellIn < 0) { that.updatePostExpiry(item);}
      }
    })
    return this.items;
  }
